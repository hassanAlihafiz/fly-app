...
export default function BookingTripDeliveryScreen() {
  ...
  React.useEffect(() => {
    if (bookingData?.bookingStatus == "delivery") {
  ...
  const getBooking = async () => {
    await getLocalStorage("booking_data")
      .then((e) => {
        ...
        setCustomerLocation({
          latitude: Number(e?.userData?.location?.lat),
          longitude: Number(e?.userData?.location?.lng),
        });
        setLoader(false);
      })
